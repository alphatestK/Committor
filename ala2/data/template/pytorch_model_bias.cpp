...
void PytorchModelBias::registerKeywords(Keywords& keys) {
  ...
  keys.add("optional","ALPHA","Fraction of the barrier to be deposited at each iteration");
...
torch::Tensor log_grad_sq = alpha* torch::log( torch::sum( torch::pow(gradient, 2) ) + 0.0000001 );
